...
void RenderHandler::OnPaint(CefRefPtr<CefBrowser> browser,
  ...
  ScopedJNIBrowser jbrowser(env, browser);
  jboolean jIsPopup = type == PET_POPUP ? JNI_TRUE : JNI_FALSE;
...
void RenderHandler::OnAcceleratedPaint(
  ...
  const cef_accelerated_paint_info_common_t extraa = info.extra;
  if (extraa.coded_size != extraa.source_size) {
    return;
  }
//  LOG(ERROR) << "OnAcceleratedPaint begin";
//  LOG(ERROR) << "type: " << info.size;
//  cef_accelerated_paint_info_common_t extra = info.extra;
//  LOG(ERROR) << "extra.size: " << extra.size;
//  LOG(ERROR) << "extra.timestamp: " << extra.timestamp;
//  LOG(ERROR) << "extra.capture_update_rect: " << extra.capture_counter;
//  LOG(ERROR) << "extra.has_capture_update_rect: " << extra.has_capture_update_rect;
//  LOG(ERROR) << "coded_size.size:           " << toString(extra.coded_size);
//  LOG(ERROR) << "visible_rect.size:         " << toString(extra.visible_rect);
//  LOG(ERROR) << "content_rect.size:         " << toString(extra.content_rect);
//  LOG(ERROR) << "source_size.size:          " << toString(extra.source_size);
//  LOG(ERROR) << "capture_update_rect.size:  " << toString(extra.capture_update_rect);
//  LOG(ERROR) << "region_capture_rect.size:  " << toString(extra.region_capture_rect);
  ...
  LOG(ERROR) << "OnAcceleratedPaint end";
}
