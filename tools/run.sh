...
if [ -z "$1" ]; then
  ...
  if [ -z "$2" ]; then
    ...
    $JAVA_HOME/bin/java -cp $OUT_PATH/jcef-tests.jar --add-exports java.desktop/sun.lwawt.macosx=jcef --add-exports java.desktop/sun.lwawt=jcef --add-exports java.desktop/sun.awt=jcef tests.$RUN_TYPE.MainFrame "$@"
  fi
fi
